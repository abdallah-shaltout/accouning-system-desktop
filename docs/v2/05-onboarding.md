...
- **استكشف ببيانات تجريبية** (explore with demo data): loads today's 75-day demo seed. A "بيانات
  تجريبية" badge stays in the topbar, and Settings → _Reset_ returns to the welcome screen.
...
| 6   | **شجرة الحسابات** (chart of accounts)                    |  ✓   | Choose مبسّط / **قياسي** (recommended) / مفصّل with a live tree preview. Name the bank accounts, one row each (bank, IBAN, currency)                                                                |
