...
| 20 | [plans/completed/20-named-route-objects/](../../plans/completed/20-named-route-objects/README.md) | Every navigation target is a named route object (`{ name, params, query, hash }`), with a generated typed route map, an `AppRoute` type, and a `check-routes.js` guard |
