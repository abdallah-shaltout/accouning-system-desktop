...
- **Automatic issues from the dev loop (18.G).** `bun run scripts/diagnostics/run.ts --ingest
  ...
  `--no-ledger` skips this). A finding that matches an existing fingerprint bumps `occurrences`/
  `last_seen` and reopens a `fixed`/`wontfix` issue (a `verified` one is left alone — that needs a
  human, since its regression test just failed). `AGENT_MEMORY.md`'s "Open diagnostics" section
  (generated by `scripts/memory/analyze/diagnostics.ts`) lists every open issue by kind/area/debug
  namespace — check it before starting work in an area that already has one.
- **Perf baseline.** `docs/diagnostics/perf-baseline.json` holds each e2e flow's last-accepted
  duration and the last-accepted `dist/` bundle size. `python scripts/e2e/run.py --update-baseline`
  accepts the current numbers as the new baseline (do this after a deliberate, reviewed change, not
  to silence a real regression). Thresholds: 25% slower for a flow, 15% bigger for the bundle —
  bundle size is far less noisy than wall-clock timing, so a smaller real jump is still worth a look.
...
bun run diag:check            # issue ledger (docs/diagnostics/ISSUES.md) must not be stale
...
- All e2e flows green with **no console errors**. Run the full suite at the end of every phase —
  partial runs have missed real regressions before. `scripts/e2e/run.py` (18.G) writes each flow's
  diagnostics export to `.diagnostics/runs/<ts>/<flow>.jsonl`, compares its duration to
  `docs/diagnostics/perf-baseline.json` (>25% slower → a `PERF-` issue; `--update-baseline` accepts a
  deliberate change), and turns any exported error or failed assertion into a `BUG-`/`PERF-` ledger
