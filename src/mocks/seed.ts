...
import { credentialsFixture, customersFixture, supplierByCategory, suppliersFixture, usersFixture } from './fixtures/people';
...
import { clone, createRandom, round2 } from './utils';
...
export function seedDatabase(now = new Date()): void {
  ...
  const refundReasons = ['المقاس غير مناسب', 'عيب في الخياطة', 'تغيير رأي العميل', 'اللون مختلف عن المتوقع'];
  ...
  for (let d = 1; d <= HISTORY_DAYS; d++) {
    ...
    if (d % 8 === 3 && !isToday) {
      const t = at(day, 9, 30);
      events.push({ time: t, run: () => restock(t) });
    }
    ...
    if (d === 30 || d === 60) {
      ...
      events.push({ time: t, run: () => stocktake(t, categoryId) });
    }
    if (d % 21 === 7) {
      const t = at(day, 21, 45);
      events.push({ time: t, run: () => writeOff(t) });
    }
  ...
  function collect(time: string, day: Date) {
  ...
  function paySuppliers(time: string, day: Date) {
  ...
  function refund(time: string) {
    ...
    recordRefund({ invoiceId: inv.id, reason: rnd.pick(refundReasons), lines: [{ invoiceLineId: line.id, qty: 1 }] }, MANAGER, time);
  }
  ...
  function stocktake(time: string, categoryId: string) {
    recordStockAdjustment(
      {
        type: 'STOCKTAKE',
        date: time,
