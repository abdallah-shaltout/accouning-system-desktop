...
/** A brand-new, empty `MockDb` — the same shape `db` starts life as. Shared by the module-level
 * initial value below and `resetDb()`, so the two can never drift apart. */
function blankDb(): MockDb {
  return {
    users: [],
    credentials: {},
    accounts: [],
...
export const db: MockDb = blankDb();

/**
 * v2 doc 18.D: resets every field of `db` back to a blank slate, in place (can't reassign the
 * `const db` binding itself — every other module imported the object reference). Needed now that
 * `scripts/verify/run.ts` calls `seedDatabase()` more than once in the same process (once per
 * country) — without this, the second seed's replay would post on top of the first's leftover
 * accounts/branches/history instead of starting fresh, doubling every total and dangling every
 * branch/cost-center id the first seed created.
 */
