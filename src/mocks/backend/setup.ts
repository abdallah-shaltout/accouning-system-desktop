...
export function applyBusinessTypeUnitDefaults(businessType: string): void {
    ...
    const extra =
        businessType === "pharmacy"
            ? [
                  { id: uid("unit"), name: "علبة", symbol: "box" },
                  { id: uid("unit"), name: "شريط", symbol: "strip" },
              ]
...
export function applyCoaTemplate(
    ...
    if (db.journalEntries.length > 0)
        throw new ApiError(
            "لا يمكن تغيير شجرة الحسابات بعد بدء الترحيل",
            "FORBIDDEN",
        );
...
export function setFiscalYear(
    ...
    if (start > ref)
        start = new Date(ref.getFullYear() - 1, startMonth - 1, startDay);
...
export function applyBranches(
    ...
    mutate(() => {
        const main =
            db.branches.find((b) => b.id === "branch-main") ?? db.branches[0];
        if (main) {
