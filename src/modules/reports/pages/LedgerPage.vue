...
const initialKind: Kind = route.query.customer
    ? "customer"
...
const targetId = ref<string | undefined>(
    (route.query[initialKind] as string | undefined) ?? undefined,
);
...
const options = computed(() => {
    const list =
        kind.value === "account"
            ? targets.value?.accounts
            : kind.value === "customer"
              ? targets.value?.customers
              : targets.value?.suppliers;
...
async function load() {
    ...
    try {
        ...
        data.value =
            kind.value === "account"
                ? await getAccountLedger(targetId.value, range)
                : await getPartyLedger(kind.value, targetId.value, range);
    } catch (err) {
    ...
    return {
        ...
        blocks: [
            banner(
                d.title,
                d.subtitle,
                `طبيعة الرصيد: ${d.normalSide === "DEBIT" ? "مدين" : "دائن"}`,
            ),
            kpis([
...
<template>
    ...
    >
        ...
        <EmptyState
            v-if="!targetId"
            :icon="BookText"
            title="اختر ما تريد عرض كشف حسابه"
            description="حساب من شجرة الحسابات، أو عميل، أو مورد"
        />
        <div
            v-else-if="data"
            ...
            <table class="w-full text-body">
                ...
                <tbody>
                    ...
                    >
                        ...
                        <td class="px-3 py-2">
                            <RouterLink
                                v-if="data.rowLinks[r.id]"
                                :to="data.rowLinks[r.id]"
                                class="num text-primary hover:underline"
                                >{{ r.entryNumber }}</RouterLink
                            >
                        </td>
                        <td class="px-3 py-2">{{ r.description }}</td>
                        <td class="px-3 py-2">
                            <MoneyText :value="r.debit" plain dash-zero />
                        </td>
                        <td class="px-3 py-2">
                            <MoneyText :value="r.credit" plain dash-zero />
                        </td>
                        <td class="px-3 py-2">
                            <MoneyText
                                :value="r.balance"
                                plain
                                :class="r.balance < 0 && 'text-danger'"
                            />
                        </td>
                    </tr>
                    <tr v-if="!data.rows.length">
                        <td
                            colspan="6"
                            class="px-3 py-6 text-center text-xs text-text-secondary"
                        >
                ...
                <tfoot class="border-t border-border bg-surface font-medium">
                    <tr>
                        ...
                        <td class="px-3 py-2.5">
                            <MoneyText :value="data.totalDebit" plain />
                        </td>
                        <td class="px-3 py-2.5">
                            <MoneyText :value="data.totalCredit" plain />
                        </td>
                        <td class="px-3 py-2.5">
                            <MoneyText :value="data.closingBalance" />
                        </td>
                    </tr>
                </tfoot>
            </table>
