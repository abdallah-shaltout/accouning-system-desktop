...
const summaryRows = computed(() => [
    ...
    { label: "عدد الفروع", value: String(props.state.branches.length) },
    {
        label: "شجرة الحسابات",
        value:
            props.state.coa.template === "basic"
                ? "مبسّط"
                : props.state.coa.template === "detailed"
                  ? "مفصّل"
                  : "قياسي",
    },
    {
        ...
        value: String(
            props.state.paymentMethods.filter((m) => m.active).length,
        ),
    },
]);
</script>
...
<template>
    <div class="space-y-4">
        ...
        <AppCard title="الأرصدة الافتتاحية">
            <div
                v-if="state.openingDone"
                class="flex items-center gap-2 text-xs text-success"
            >
                <Check class="size-4" /> تم ترحيل القيد الافتتاحي — رصيد حساب
                الأرصدة الافتتاحية (3900):
                <MoneyText :value="openingEquityNet" class="num" />
            </div>
            <p v-else class="text-xs text-text-secondary">
                لم تُدخل الأرصدة الافتتاحية بعد — يمكن إتمامها لاحقاً من بطاقة
                "الإعداد" في لوحة التحكم أو من /setup/opening.
            </p>
        </AppCard>

        <p class="text-xs text-text-secondary">
            اضغط "ابدأ العمل" لإنهاء الإعداد والانتقال إلى صفحة تسجيل الدخول.
        </p>
    </div>
</template>
