...
const fontOptions: { value: FontFamily; label: string }[] = Object.entries(
    FONTS,
).map(([value, def]) => ({
    value: value as FontFamily,
    label: def.label,
}));
const textSizeOptions: { value: TextSize; label: string }[] = TEXT_SIZES.map(
    (v) => ({ value: v, label: `${v}%` }),
);
const densityOptions: { value: Density; label: string; hint: string }[] = [
    {
        value: "comfortable",
        label: "مريحة",
        hint: "تباعد أوسع، مناسب للشاشات الكبيرة",
    },
    {
        value: "compact",
        label: "مضغوطة",
        hint: "صفوف أقصر، بيانات أكثر في الشاشة",
    },
];
const accentOptions: { value: AccentPreset; label: string }[] = Object.entries(
    ACCENTS,
).map(([value, def]) => ({
    value: value as AccentPreset,
    label: def.label,
}));
const baseOptions: { value: BasePalette; label: string }[] = Object.entries(
    BASES,
).map(([value, def]) => ({
    value: value as BasePalette,
    label: def.label,
}));
const radiusOptions: { value: ThemeRadius; label: string }[] = RADII.map(
    (r) => ({ value: r, label: r === 0 ? "0" : String(r) }),
);
const presetOptions = Object.entries(THEME_PRESETS).map(([key, def]) => ({
    key: key as keyof typeof THEME_PRESETS,
    ...def,
}));
...
const isActivePreset = (key: keyof typeof THEME_PRESETS) => {
    ...
    return (
        preset.base === base.value &&
        preset.accent === accent.value &&
    ...
    <div>
        ...
        <div class="max-w-3xl space-y-5">
                ...
                <div class="grid gap-3 sm:grid-cols-3">
                    ...
                    >
                        ...
                        <span
                            ...
                            :class="
                                t.value === 'dark'
                                    ? 'border-[#2d2e33] bg-[#0e0f11]'
                                    : t.value === 'light'
                                      ? 'border-[#d4d4d8] bg-white'
                                      : 'border-border bg-linear-to-l from-white to-[#0e0f11]'
                            "
                        >
                            <span
                                class="w-1/4"
                        ...
                        <span
                            class="flex items-center gap-1.5 text-body"
                            :class="
                                themeMode === t.value &&
            ...
            <AppCard title="شكل الأرقام">
                <div class="grid gap-3 sm:grid-cols-2">
                    ...
                    >
                        ...
                        >
                        <span
                            class="num mt-1 block text-lg text-text-secondary"
                            >{{
                                n.value === "latn" ? "1,234.50" : "١٬٢٣٤٫٥٠"
                            }}</span
                        >
                    </button>
                </div>
            ...
            <AppCard title="نوع الخط">
                <div class="grid gap-3 sm:grid-cols-2">
                    ...
                    >
                        ...
                        >
                        <span
                            class="mt-1 block text-lg text-text-secondary"
                            :style="{
                                fontFamily: `'${FONTS[f.value].cssName}', sans-serif`,
                            }"
                        >
                ...
                <p class="mt-3 text-xs text-text-secondary">
                    يُحمَّل خط الاختيار عند الحاجة فقط (بدون إنترنت)، ولا
                    تُحمَّل بقية الخطوط.
                </p>
            </AppCard>
            ...
            <AppCard title="الثيم والألوان">
                ...
                <div class="space-y-5">
                    <div>
                        <p class="mb-2 text-label text-text-secondary">
                            قوالب جاهزة
                        </p>
                        <div class="grid grid-cols-2 gap-3 sm:grid-cols-4">
                            ...
                            >
                                <span
                                    class="flex h-8 w-full overflow-hidden rounded-md border border-border"
                                    :style="{
                                        background: BASES[p.base].swatch,
                                    }"
                                >
                                    <span
                                        class="h-full w-1/3"
                                        :style="{
                                            background: ACCENTS[p.accent].light,
                                            borderRadius: `${p.radius}rem 0 0 ${p.radius}rem`,
                                        }"
                                    />
                                </span>
                                <span
                            ...
                            >
                                ...
                                >
                                    <span
                                        class="w-1/2"
                                        :style="{
                                            background: BASES[b.value].swatch,
                                        }"
                                    />
                                    <span
                                        class="w-1/2"
                        ...
                        <div class="grid grid-cols-2 gap-3 sm:grid-cols-4">
                            ...
                            >
                                <span
                                    ...
                                    :style="{
                                        background: ACCENTS[a.value].light,
                                        color: ACCENTS[a.value].onAccent,
                                    }"
                                >
                                    Aa
                                </span>
                    ...
                    <div>
                        ...
                        <div
                            ...
                            :style="{ '--preview-radius': previewRadius }"
                        >
                            <div class="flex flex-wrap items-center gap-3">
                                <AppButton
                                    variant="primary"
                                    style="border-radius: var(--preview-radius)"
                                    >حفظ</AppButton
                                >
                                <AppButton
                                    variant="secondary"
                            ...
                            <div
                                class="mt-3 overflow-hidden border border-border"
                                style="border-radius: var(--preview-radius)"
                            >
                                <div
                                    class="grid grid-cols-3 bg-surface-hover px-3 py-2 text-label text-text-secondary"
                                >
                                    <span>الفاتورة</span><span>العميل</span
                                    ><span>الإجمالي</span>
                                </div>
                                <div
                                    class="grid grid-cols-3 border-t border-border px-3 py-2 text-body"
                                >
                                    <span class="num text-primary"
                                        >INV-1042</span
                                    ><span>محمد علي</span
                                    ><span class="num">1,250.00</span>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <AppModal
                ...
                title="معاينة النافذة"
                description="مثال حي يعكس استدارة الحواف والألوان المختارة"
            >
                <p class="text-body text-text-secondary">
                    هكذا تبدو نافذة حوار بالثيم الحالي.
                </p>
                <template #footer>
                    <AppButton
                        variant="secondary"
