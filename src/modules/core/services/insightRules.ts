...
const reorderRule: InsightRule = (ctx) => {
  ...
  for (const [supplierId, items] of bySupplier) {
    ...
    insights.push({
      ...
      actionTo: supplier ? { name: 'purchase-new', query: { supplier: supplierId } } : { name: 'products', query: { stock: 'low' } },
      icon: PackageX,
...
const overdueCustomersRule: InsightRule = (ctx) => {
  ...
  for (const [customerId, { total, maxDays }] of byCustomer) {
    ...
    insights.push({
      ...
      actionTo: { name: 'customer', params: { id: customerId } },
      icon: UserX,
...
const creditLimitRule: InsightRule = (ctx) => {
  ...
  for (const c of db.customers) {
    ...
    insights.push({
      ...
      actionTo: { name: 'customer', params: { id: c.id } },
      icon: CreditCard,
  ...
  return [
    {
      ...
      actionTo: { name: 'card-settlements' },
      icon: CreditCard,
...
const recurringJournalDueRule: InsightRule = (ctx) => {
  ...
  return due.map((t) => ({
    ...
    actionTo: { name: 'journal-templates' },
    icon: Repeat,
...
const budgetRule: InsightRule = (ctx) => {
  ...
  for (const c of db.costCenters) {
    ...
    insights.push({
      ...
      actionTo: { name: 'report-cost-centers' },
      icon: PiggyBank,
      roles: ['manager', 'admin'] as const,
      value: actual,
      createdAt: ctx.today,
