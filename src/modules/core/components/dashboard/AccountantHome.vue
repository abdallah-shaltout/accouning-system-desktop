  ...
  <div>
    <div class="mb-5 flex flex-wrap items-end justify-between gap-3">
      ...
      <div class="flex gap-2">
        <AppButton :icon="FileText" :to="{ name: 'journal-new' }">قيد جديد</AppButton>
        <AppButton variant="primary" :icon="ReceiptText" :to="{ name: 'expenses' }">مصروف جديد</AppButton>
      </div>
    </div>
    ...
    <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      <KpiCard label="الصندوق" :icon="Banknote" :loading="summary.loading.value" :to="{ name: 'report-ledger', query: { account: 'acc-1110' } }">
        <span dir="ltr">{{ formatMoney(summary.data.value?.cashOnHand) }}</span> <RiyalIcon class="text-[0.7em] text-text-secondary" />
      </KpiCard>
      <KpiCard label="البنك" :icon="Landmark" :loading="summary.loading.value" :to="{ name: 'report-ledger', query: { account: 'acc-1120' } }">
        <span dir="ltr">{{ formatMoney(summary.data.value?.bankBalance) }}</span> <RiyalIcon class="text-[0.7em] text-text-secondary" />
      </KpiCard>
      <KpiCard label="فواتير غير مسددة" :icon="ReceiptText" :loading="summary.loading.value" :to="{ name: 'invoices', query: { payment: 'open' } }">
      ...
      <KpiCard label="مسودات بحاجة لترحيل" :icon="FileText" :loading="summary.loading.value" :tone="draftCount > 0 ? 'warning' : undefined" :to="{ name: 'journal' }">
        {{ draftCount }}
      </KpiCard>
    </div>
    ...
    <div class="mt-4 grid gap-4 lg:grid-cols-2">
      <AppCard title="تسوية ضريبة القيمة المضافة" padding="none">
        <RouterLink :to="{ name: 'vat-settlement' }" class="flex items-center justify-between gap-3 px-4 py-3 hover:bg-surface-hover">
          <span class="text-body">عرض التسوية والموعد النهائي</span>
        </RouterLink>
      </AppCard>
      <AppCard title="الذمم والتقارير" padding="none">
        <div class="flex flex-wrap gap-2 p-4">
          <AppButton size="sm" :to="{ name: 'customers' }">أرصدة العملاء</AppButton>
          <AppButton size="sm" :to="{ name: 'suppliers' }">أرصدة الموردين</AppButton>
          <AppButton size="sm" :to="{ name: 'reports' }">كل التقارير</AppButton>
        </div>
      </AppCard>
    </div>
  </div>
