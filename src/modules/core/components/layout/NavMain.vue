...
<template>
  <SidebarGroup>
    <SidebarMenu>
      <template v-for="group in groups" :key="group.label ?? group.items[0].to">
        ...
        <SidebarMenuItem v-else-if="state === 'collapsed'">
          <DropdownMenu>
            ...
            <DropdownMenuContent side="left" align="start" class="min-w-48">
              <DropdownMenuItem v-for="item in group.items" :key="item.to" as-child>
        ...
        <Collapsible v-else :key="`c-${group.label}`" as-child :open="openGroup === group.label" @update:open="() => toggleGroup(group.label!)">
          <SidebarMenuItem>
            ...
            <CollapsibleContent>
              <SidebarMenuSub>
                <SidebarMenuSubItem v-for="item in group.items" :key="item.to">
